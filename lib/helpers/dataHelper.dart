...
Future getAllItems() async {
  ...
  var url = "https://littlewordsapi.french-heberg.com/word/around?latitude=${location.latitude}&longitude=${location.longitude}";
...
Future<http.Response> postRequest (word,user,lat,long) async {
  var url = "https://littlewordsapi.french-heberg.com/word?";
  Map data = {
	  "content":word,
	  "author":user,
	  "latitude": lat,
...
Future getDetails(uid, latitude, longitude) async {
  var url = "https://littlewordsapi.french-heberg.com/word?uid=${uid}&latitude=${latitude}&longitude=${longitude}";
  var data = await fetchData(url);
  return data;
}
